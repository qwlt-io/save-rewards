...
const { v4: uuidv4, parse: uuidParse, stringify: uuidStringify, validate: uuidValidate } = require('uuid')
...
export default class Cassandra{
    ...
    constructor(){
        const certPath = path.join(__dirname, '../static/certs/sf-class2-root.crt');
    ...
    async isFirstTimeLoginForTheDay(userId){
        try{
            let query = await this.getQuery(CASSANDRA_TABLES.USER_APP_OPENINGS, 'SELECT');
    ...
    async logFirstTimeLoginForTheDay(userId){
        try{
            ...
            let query = await this.getQuery(CASSANDRA_TABLES.USER_APP_OPENINGS, 'INSERT');
    ...
    public async executeInsertQueryForStepCount(rewardPointsParamsForStepCount){
        let {userId, stepCount, createdAt, updatedAt} = rewardPointsParamsForStepCount
        let id = uuidv4();
    ...
    public async executeInsertQueryForWalletTransactions(walletTransactionInput){
        ...
        const {id, userId, walletId,txnType, coinType, rewardPoints, rewardTypeId, createdAt, updatedAt } = walletTransactionInput;
        
        const query = await this.getQuery(CASSANDRA_TABLES.WALLET_TRANSACTION);
        const params = [id, userId, walletId, txnType, rewardPoints, coinType, rewardTypeId, createdAt, updatedAt]
